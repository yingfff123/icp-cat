(function () {
  ...
  async function handleListResponse(response, init) {
    ...
    const detailTargets = list
      .filter((item) => item && item.dataId)
      .map((item) => ({
        dataId: item.dataId,
        serviceType: normalizeServiceType(item.serviceType),
        originalServiceType: item.serviceType
      }));
    ...
    for (const target of detailTargets) {
      const key = `${target.serviceType || "unknown"}:${target.dataId}`;
      ...
      try {
        const detailBody = {
          ...
          ...(target.serviceType ? { serviceType: target.serviceType } : {})
        };
